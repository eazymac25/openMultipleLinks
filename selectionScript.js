(function() {
	...
	function mouseMove(e) {
		...
		var minCoor = new coord(Math.min(movedCoord.x,box.firstCoord.x),
								Math.min(movedCoord.y,box.firstCoord.y));

		var maxCoor = new coord(Math.max(movedCoord.x,box.firstCoord.x),
								Math.max(movedCoord.y,box.firstCoord.y));
	...
	function overlap(allLinks,minCoor,maxCoor){
		...
		for (var i = 0; i<allLinks.length; i++){
			...
			var topLeft = new coord(bb.left + pageXOffset, bb.top + pageYOffset);
			var botRight = new coord(bb.right + pageXOffset,bb.bottom + pageYOffset);
	...
	function isValidLink(link) {
		/*
		TODO: Need to implement function and check if a given link
		is actually a hpyerlink or just an anchor point
		*/
		return false;
	}
